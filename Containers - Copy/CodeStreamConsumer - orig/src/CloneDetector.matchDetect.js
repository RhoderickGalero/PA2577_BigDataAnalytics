...
class CloneDetector {
   ...
   #filterCloneCandidates(file, compareFile) {
        ...
        // Find all matching chunks and create Clone instances
        const newInstances = file.chunks
            .map(chunk => {
                const matchingChunks = compareFile.chunks.filter(compareChunk => chunkMatch(chunk, compareChunk));
                return matchingChunks.map(matchingChunk => new Clone(matchingChunk));
            })
            .flat();
    ...
    #expandCloneCandidates(file) {
        ...
        // Use reduce to expand Clones in file.instances
        file.instances = file.instances.reduce((accumulator, currentClone) => {
        // Check if the currentClone overlaps with any Clone in the accumulator
        const overlaps = accumulator.some(existingClone => {
            // TODO: Implement logic to check for overlap (you might use your own method)
            return false; // Replace with your overlap-checking logic
        });

        // If there is an overlap, expand the existing Clone; otherwise, add the current Clone to the accumulator
        if (overlaps) {
            accumulator.forEach(existingClone => {
    ...
    #consolidateClones(file) {
        ...
        //
         ...
         // Use reduce to consolidate Clones in file.instances
        file.instances = file.instances.reduce((accumulator, currentClone) => {
        const existingClone = accumulator.find(clone => clone.equals(currentClone));

        if (existingClone) {
            // If the current Clone is not new, update the existing Clone
            existingClone.addTarget(currentClone);
        } else {
            // If the current Clone is new, push it into the accumulator
            accumulator.push(currentClone);
        }
...
//            file = this.#consolidateClones(file);
            ...
            // Step 1: Find all equal chunks in file and f. Represent each matching pair as a Clone.
            file = this.#filterCloneCandidates(file, f);

            // Step 2: For each Clone with endLine=x, merge it with Clone with endLine-1=x
            //         remove the now redundant clone, rinse & repeat.
            file.instances = file.instances.reduce((accumulator, currentClone) => {
                let merged = false;
                for (let existingClone of accumulator) {
                    if (existingClone.endLine === currentClone.endLine - 1) {
                        existingClone.maybeExpandWith(currentClone);
                        merged = true;
                        break;
                }
            ...
            // Step 3: If the same clone is found in several places, consolidate them into one Clone.
            file = this.#consolidateClones(file);
