...
function viewTimers(req, res, next) {
    let timers = Timer.getFileTimers();
    let page = '<HTML><HEAD><TITLE>CodeStream Clone Detector - Timers</TITLE></HEAD>\n';
    page += '<BODY><H1>CodeStream Clone Detector - Timers</H1>\n';
    
    for (const fileName in timers) {
        page += '<p>File: ' + fileName + '<p>\n';
        page += '<ul>\n';
        for (const timerName in timers[fileName]) {
            page += '<li>' + timerName + ': ' + (timers[fileName][timerName] / (1000n)) + ' µs\n';
        }
        page += '</ul>\n';
    }
...
function shouldPrintStatistics() {
    // Define your condition here based on your requirements, e.g., every 10th file
    return cloneDetector.numberOfProcessedFiles % STATS_FREQ === 0;
}
/*
function maybePrintStatistics(file, cloneDetector, cloneStore) {
