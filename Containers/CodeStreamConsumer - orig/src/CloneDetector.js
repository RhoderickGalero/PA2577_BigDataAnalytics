...
class CloneDetector {
    ...
    #filterCloneCandidates(file, compareFile) {
        ...
        file.chunks.forEach((chunk) => {
            ...
            const newInstances = matchingChunks.map(matchingChunk => {
                const startLine = chunk[0].lineNumber;
                const endLine = chunk[chunk.length - 1].lineNumber;
                return new Clone(startLine, endLine, compareFile.name);
            });
