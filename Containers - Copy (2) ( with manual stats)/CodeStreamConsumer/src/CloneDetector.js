...
class CloneDetector {
    ...
    #filterCloneCandidates(file, compareFile) {
        ...
        // Iterate through each chunk in file.chunks
        for (let chunk of file.chunks) {
            // Find all matching chunks in compareFile.chunk
            let matchingChunks = compareFile.chunks.filter((compareChunk) => {
                return this.#chunkMatch(chunk, compareChunk);
            });

            // Create a new Clone for each matching chunk
            let clones = matchingChunks.map((matchingChunk) => {
                let sourceName = file.name;
                let targetName = compareFile.name;
                let sourceChunk = chunk;
                let targetChunk = matchingChunk;

                return new Clone(sourceName, targetName, sourceChunk, targetChunk);
            });

            // Append the resulting clones to file.instances
            file.instances = file.instances.concat(clones);
        }
    ...
    #expandCloneCandidates(file) {
        ...
        for (let i = 0; i < file.instances.length; i++) {
            ...
            for (let j = 0; j < expandedClones.length; j++) {
                let existingClone = expandedClones[j];

                // Check if the currentClone can be expanded with the existingClone
                if (currentClone.maybeExpandWith(existingClone)) {
                    canExpand = false;
                    break; // No need to check further if expanded
                }
            }

            // If currentClone couldn't be expanded with any existingClone, add it to the expandedClones
            if (canExpand) {
    ...
    #consolidateClones(file) {
        ...
        for (let i = 0; i < file.instances.length; i++) {
            ...
            for (let j = 0; j < uniqueClones.length; j++) {
                let existingClone = uniqueClones[j];
    
                // Check if the currentClone is equal to an existingClone
                if (currentClone.equals(existingClone)) {
                    // Update the existingClone by adding the targets from the currentClone
            ...
            // If currentClone is new, add it to the uniqueClones array
            if (isNewClone) {
    ...
    matchDetect(file) {
        ...
        let prevInstancesCount = -1;
        while (file.instances.length !== prevInstancesCount) {
            prevInstancesCount = file.instances.length;
